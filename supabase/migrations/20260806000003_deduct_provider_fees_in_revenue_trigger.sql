-- =============================================================================
-- Migration: Deduct payment-provider fees inside the revenue-capture trigger
--
-- The trigger on payments uses the nominal platform/ticketing fee.  This update
-- makes it subtract the configured provider fee from the platform portion so
-- the organizer still pays the configured platform fee, but the gateway charge
...
BEGIN
  ...
  -- Subtract the payment provider's service charge from the platform's portion.
  ...
  END;
  v_platform_fee_percent := GREATEST(v_platform_fee_percent - v_provider_fee_percent, 0);
