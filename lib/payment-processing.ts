...
const CANONICAL_PAID_PAYMENT_STATUS = 'completed'
...
const paystackInitSchema = z
  .object({
    ...
    votes: z.coerce.number().int().min(1).max(10000).optional(),
...
async function createVoteFallback(params: {
  ...
  if (voteEventId && voteCandidateId) {
    const { error: rpcError } = await supabase.rpc('process_vote', {
      ...
      p_voter_id: (payment.voter_id ?? payment.user_id) || null,
  ...
  if (!insertedVoteId) {
    const basePayload = {
      ...
      voter_id: (payment.voter_id ?? payment.user_id) || null,
